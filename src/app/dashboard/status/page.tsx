...
export default async function StatusPage() {
  ...
  let servicesOk = {
    ...
    twilio_envs: undefined as boolean | undefined,
  ...
  if (resp.kind === "health") {
    ...
    servicesOk = {
      ...
      twilio_envs: Boolean(d?.services?.twilio?.envs),
  ...
  const Item = ({ label, value }: { label: string; value: boolean | undefined }) => (
        ...
        <div className="rounded-2xl bg-background p-5 shadow-sm">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="rounded-lg border p-4">
              ...
              <Item label="Twilio envs" value={servicesOk.twilio_envs} />
              <Item label="Worker (heartbeat)" value={servicesOk.worker_heartbeat} />
            </div>

            <div className="rounded-lg border p-4">
              <h3 className="text-sm font-semibold mb-3">Entorno</h3>
              <Item label="NEXT_PUBLIC_SUPABASE_URL" value={envOk.NEXT_PUBLIC_SUPABASE_URL} />
              <Item label="SUPABASE_SERVICE_ROLE_KEY" value={envOk.SUPABASE_SERVICE_ROLE_KEY} />
                  ...
                  <div key={name} className="rounded-md border p-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {resp.kind === "queues" && (resp.data as any)?.[name] === "up" ? "up" : ""}
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>waiting: <b>{q?.waiting ?? 0}</b></div>
                      <div>active: <b>{q?.active ?? 0}</b></div>
                      <div>completed: <b>{q?.completed ?? 0}</b></div>
                      <div>failed: <b>{q?.failed ?? 0}</b></div>
                      <div>delayed: <b>{q?.delayed ?? 0}</b></div>
                      <div>paused: <b>{q?.paused ?? 0}</b></div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {errorMsg && <p className="mt-4 text-sm text-red-600">Error: {errorMsg}</p>}
          <div className="flex gap-3 mt-4">
            <a href="/dashboard" className="text-sm underline">← Volver</a>
            <a href="/api/health" className="text-sm underline">Ver JSON /api/health</a>
            <a href="/api/admin/status" className="text-sm underline">Ver JSON /api/admin/status</a>
          </div>
        </div>
      </div>
    </div>
